...
Puppet::Type.newtype(:sdb_mysql_user) do
  ...
  validate do
    fail('name must be a combination of instance_name and user_host, separated by a space') if self[:name] != "#{self[:instance_name]} #{self[:user_host]}"
  end
  ...
  newproperty(:instance_name) do
    desc 'Name of the mysql instance.'
    newvalue(/\w*/)
  end
  
  newproperty(:defaults_file) do
    desc 'Path of defaults file parameter --defaults-extra-file.'
    newvalue(/\w*/)
  end
