...
Puppet::Type.type(:sdb_mysql_user).provide(:sdb_mysql, :parent => Puppet::Provider::Sdb_Mysql) do
  ...
  def self.instances
    ...
    @mysql_instances.each do |instance_name, defaults_file|
      ...
      # check that root user exists, if not then create dummy instance
      # this is only necessary for the first run when the mysql instance not exists
      if is_defaults_file_with_root_pw(instance_name) == false
        return_value << new(
          :name                     => instance_name + " " + "root@localhost",
      ...
      users.collect do |name|
        ...
        @max_updates_per_hour, @password, @plugin = mysql([mysql_defaults_file_params, "-NBe", query].compact).split(/\s/)
  ...
  def create
    name                     = @resource[:name]
    merged_name              = @resource[:user_host].sub('@', "'@'")
    ...
    if !plugin.nil?
      mysql([mysql_defaults_file_params, '-e', "CREATE USER '#{merged_name}' IDENTIFIED WITH '#{plugin}'"].compact)
      @property_hash[:ensure] = :present
      @property_hash[:plugin] = plugin
    else
      mysql([mysql_defaults_file_params, '-e', "CREATE USER '#{merged_name}' IDENTIFIED BY PASSWORD '#{password_hash}'"].compact)
      @property_hash[:ensure] = :present
      @property_hash[:password_hash] = password_hash
    end
    mysql([mysql_defaults_file_params, '-e', "GRANT USAGE ON *.* TO '#{merged_name}' WITH MAX_USER_CONNECTIONS #{max_user_connections} MAX_CONNECTIONS_PER_HOUR #{max_connections_per_hour} MAX_QUERIES_PER_HOUR #{max_queries_per_hour} MAX_UPDATES_PER_HOUR #{max_updates_per_hour}"].compact)
  ...
  def destroy
    ...
    mysql([mysql_defaults_file_params, '-e', "DROP USER '#{merged_name}'"].compact)
  ...
  def password_hash=(string)
    merged_name = self.class.cmd_user(@resource[:user_host])
    mysql_defaults_file_params = self.class.defaults_file(@resource.value(:instance_name), @resource.value(:defaults_file))

    mysql([mysql_defaults_file_params, '-e', "SET PASSWORD FOR #{merged_name} = '#{string}'"].compact)
  ...
  def max_user_connections=(int)
    ...
    mysql([mysql_defaults_file_params, '-e', "GRANT USAGE ON *.* TO #{merged_name} WITH MAX_USER_CONNECTIONS #{int}"].compact).chomp
  ...
  def max_connections_per_hour=(int)
    ...
    mysql([mysql_defaults_file_params, '-e', "GRANT USAGE ON *.* TO #{merged_name} WITH MAX_CONNECTIONS_PER_HOUR #{int}"].compact).chomp
  ...
  def max_queries_per_hour=(int)
    ...
    mysql([mysql_defaults_file_params, '-e', "GRANT USAGE ON *.* TO #{merged_name} WITH MAX_QUERIES_PER_HOUR #{int}"].compact).chomp
  ...
  def max_updates_per_hour=(int)
    ...
    mysql([mysql_defaults_file_params, '-e', "GRANT USAGE ON *.* TO #{merged_name} WITH MAX_UPDATES_PER_HOUR #{int}"].compact).chomp
