require File.expand_path(File.join(File.dirname(__FILE__), '..', 'sdb_mysql'))

Puppet::Type.type(:sdb_mysql_grant).provide(:sdb_mysql, :parent => Puppet::Provider::Sdb_Mysql) do

  desc 'Set grants for users in MySQL.'

  @mysql_instances = Hash.new
  def self.instances
    instances = []
    @mysql_instances.each do |instance_name, defaults_file|
      ...
      users(instance_name, defaults_file).select{ |user| user =~ /.+@/ }.collect do |user|
        ...
        begin
          grants = mysql([mysql_defaults_file_params, "-NBe", query].compact)
        ...
        grants.each_line do |grant|
          ...
          if match = munged_grant.match(/^GRANT\s(.+)\sON\s(.+)\sTO\s(.*)@(.*?)(\s.*)?$/)
            ...
            instances << new(
            :name       => "#{instance_name} #{user}@#{host}/#{table}",
            :ensure     => :present,
  ...
  def self.prefetch(resources)
    resources.keys.each do |name|
      @mysql_instances[resources[name].value(:instance_name)] = resources[name].value(:defaults_file)
    end
  ...
  def grant(user, table, privileges, options)
    mysql_defaults_file_params = self.class.defaults_file(@resource.value(:instance_name), @resource.value(:defaults_file))
    ...
    mysql([mysql_defaults_file_params, '-e', query].compact)
  end
  ...
  def flush
    ...
    @property_hash.clear
    mysql([mysql_defaults_file_params, '-NBe', 'FLUSH PRIVILEGES'].compact)
  end
